                ...
                <div class="card-header d-flex align-items-center">
                    <div>
                        ...
                        <div class="quiz-info">
                            ...
                            <span>Total Mark :
                                <small>{{ (count($quiz->questionsGradeSum) > 0) ? $quiz->questionsGradeSum[0]->grade_sum : 0 }}</small></span>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="result-card">
                            <img src="/assets/default/images/{{ $quiz_result->status == 'pass' ? 'winners.png' : 'feeling.png'}}"
                                alt="">
                            <h3 class="result-msg">
                                {{ $quiz_result->status == 'pass' ? trans('main.quiz_winners') : ($quiz_result->status == 'fail' ? trans('main.quiz_feeling') : trans('main.quiz_waiting')) }}
                            </h3>
                            @if ($quiz_result->status == 'fail' and $canTryAgain)
                            <a href="/user/quizzes/{{ $quiz->id }}/start"
                                class="btn btn-custom btn-danger">{{ trans('main.try_again') }}</a>
                            @elseif ($quiz_result->status == 'pass' and $quiz->certificate)
                            <a href="/user/certificates/{{ $quiz_result->id }}/download"
                                class="btn btn-custom btn-danger">{{ trans('main.download_certificate') }}</a>
                            @endif
                        </div>
                    </div>
                </div>
