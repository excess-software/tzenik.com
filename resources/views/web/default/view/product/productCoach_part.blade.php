        ...
        <div class="row product-body">
            <div class="container">
                <div class="col-md-4 col-xs-12 course_details">
                    ...
                    <div class="product-details-box">
                        ...
                        <span>
                            ...
                            <b>{{{trans('main.meeting_date')}}}</b> {{{ date('d-m-Y', strtotime($meeting->date)) }}} {{{ $meeting->time }}}
                            @endforeach
                            @else
                            <b>{{{trans('main.meeting_date')}}}</b> {{{ $meeting_date }}}
                            @endif
                        </span>
                    </div>
                    <div class="product-price-box">
                        <span class="proicon mdi mdi-wallet"></span>
                        @if($meta['price'] == '0' || $product->price == 0)
                    ...
                    <div class="product-buy-selection">
                        <form>
                            ...
                            @else
                                @if(!$buy)
                                    @if($product->price != 0 && $meta['price'] != '0')
                                        <div class="radio">
                                    ...
                                    @if($product->price != 0 && $meta['price'] != '0')<a class="btn btn-orange product-btn-buy sbox3" id="buy-btn" data-toggle="modal" data-target="#buyModal" href="">{{{ trans('main.pay') }}}</a>@endif
                                @else
                                    @if($product->price != 0 && $meta['price'] != '0')<a class="btn btn-orange product-btn-buy sbox3" href="javascript:void(0);">{{{ trans('main.purchased_item') }}}</a>@endif
                                @endif
                            @endif
                ...
                <div class="col-md-8 col-xs-12">
                    <iframe id="frame_zoom" scrolling="no" style="height: 500px; width: 100%;" src="https://zoom.us/wc/{{{ $meeting->zoom_meeting or $meeting }}}/join?prefer=0&un=TWluZGF1Z2Fz" sandbox="allow-forms allow-scripts allow-same-origin" allow="microphone; camera" allowfullscreen></iframe>
                </div>
            </div>
        </div>
    </div>
                        ...
                        @if(isset($ads))
                            @foreach($ads as $ad)
                                @if($ad->position == 'product-page')
                                    <a href="{{{ $ad->url or '#' }}}"><img src="{{{ $ad->image ?? '' }}}" class="{{{ $ad->size ?? '' }}}" id="ppage-s"></a>
                                @endif
                            @endforeach
                        @endif
                    </div>
                                    ...
                                    @if(isset($meta['document']) && $meta['document']!='')
                                        <li class="document">
                                            ...
                                            <div class="col-md-1 text-center">
                                                <a href="{{{ $meta['document'] ?? '' }}}"><span class="download-part" data-href=""><span class="mdi mdi-arrow-down-bold"></span></span></a>
                                            </div>
                                        </li>
                                    @endif
                                </ul>
                    ...
                    <div class="user-tabs">
                        ...
                        <div class="tab-content">
                            ...
                            @if($product->support == 1)
                                ...
                                @if($buy || $product->price == 0 || $meta['price'] == '0')
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            $('#frame_zoom').load( function () {
                $(this).contents().find("#header_container").css({'display':'none'});
            });
            var iframe = document.getElementById("frame_zoom");
            var header = iframe.document.getElementById("header_container");
