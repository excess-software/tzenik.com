                ...
                <div class="col-md-4 col-sm-6 col-xs-12">
                    <div class="box box-s">
                        <div class="container-2">
                            <form>
                                {{ csrf_field() }}
                                <input type="search" id="search" name="q" value="{{ !empty(request()->get('q')) ? request()->get('q') : '' }}" placeholder="Search in  {{ !empty($category->title) ? $category->title : 'Search in all categories' }}"/>
                                            ...
                                            @if(count($filter->tags) > 0)
                                                <ul class="submenu">
                                                    @foreach($filter->tags as $tag)
                                                        <li>
                                                            <input type="checkbox" id="filter{{ $tag->id }}" name="filters" value="{{ $tag->id }}" @if(!is_null($filters) && in_array($tag->id,$filters)) checked @endif/>
                                                            <label for="filter{{ $tag->id }}"><span></span>{{ $tag->tag }}</label>
                                                        </li>
                                    ...
                                    @if(isset($content->content->id))
                                        ...
                                        <div class="col-md-4 col-sm-6 col-xs-12 pagi-content vip-content tab-con">
                                            <a href="/product/{{ $content->content->id }}" title="{{ $content->content->title }}" class="content-box pagi-content-box">
                                                ...
                                                <div class="footer">
                                                    <span class="avatar" title="{{ $content->user->name ?? '' }}" onclick="window.location.href = '/profile/{{ $content->user->id }}'">
                                                        <img src="{{ get_user_meta($content['user_id'],'avatar',get_option('default_user_avatar','')) }}">
                                                    </span>
