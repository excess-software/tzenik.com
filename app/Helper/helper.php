...
function sendMail(array $request)
{
    ...
    if (isset($request['template'])) {
        ...
        if(isset($template)) {
            ...
            $request['subject'] = $subject != '' ? $request['subject'] : $template->title;
        }
    }

    foreach ($users as $to) {
        if(isset($request['message'])) {
            $request['message'] = str_replace(['[username]', '[name]', '[email]', '[active]', '[token]', '[n.title]', '[n.content]', '[password]'], [$to->username, $to->name, $to->email, url('/') . '/user/active/' . $to->token, url('/') . '/user/reset/token/' . $to->token, $request['title'], $request['content']], $request['message'], decrypt($to->password));
...
function sendNotification($senderId, $source, $message_id, $recipentType, $recipentList)
{
    ...
    if ($recipentType == 'user') {
        $userEmail = cache()->remember('user.' . $recipentList, 2 * 24 * 60 * 60, function () use ($recipentList) {
            return \App\User::find($recipentList);
        });
